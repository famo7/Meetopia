  ...
  <Card class="border-gray-200 shadow-sm">
    <CardHeader class="pb-4 border-b border-gray-100">
      <div class="flex items-center justify-between">
        ...
        <button @click="loadActionItems" class="p-1.5 hover:bg-gray-100 rounded-lg transition-colors"
          title="Refresh action items">
          <RefreshCw class="h-4 w-4 text-gray-500" :class="{ 'animate-spin': actionItemStore.isLoading }" />
        </button>
      </div>
    </CardHeader>
    <CardContent class="p-6">
      ...
      <div v-else-if="actionItemStore.error" class="text-center py-8">
        <p class="text-red-500 mb-3">{{ actionItemStore.error }}</p>
        <button @click="loadActionItems" class="text-sm text-blue-600 hover:text-blue-700">
          Try again
        </button>
      </div>
